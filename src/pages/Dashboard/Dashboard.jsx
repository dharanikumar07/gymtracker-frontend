...
import { useDashboardQuery } from './http/dashboardQueries';
...
const Dashboard = () => {
    ...
    const menuItems = [
        { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, component: null },
                ...
                <header className="h-16 bg-card border-b border-border flex items-center justify-between px-4 sm:px-6 shrink-0 sticky top-0 z-40">
                    <div className="flex items-center gap-4 text-foreground">
                        <h2 className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground">
                            {activeTab === 'dashboard' ? 'Home' : menuItems.find(i => i.id === activeTab)?.label}
                        </h2>
                    </div>
